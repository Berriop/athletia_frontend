...
describe('calculateRecoveryScore', () => {
  ...
  it('R2: sueño=8h, estrés=2, 0 lesiones, 4 entrenamientos → score=100, "Excelente"', () => {
    const result = calculateRecoveryScore(sleep({ hoursSlept: 8, stressLevel: 2 }), 0, workouts(4));
  ...
  it('R3: sueño=5h, estrés=8, 2 lesiones activas, 0 entrenamientos → score=25, "Riesgo"', () => {
    const result = calculateRecoveryScore(sleep({ hoursSlept: 5, stressLevel: 8 }), 2, []);
  ...
  it('R4: frontera de horas dormidas — exactamente 7h suma 30, exactamente 6h suma 20', () => {
    const at7 = calculateRecoveryScore(sleep({ hoursSlept: 7, stressLevel: 0 }), 0, []);
    const at6 = calculateRecoveryScore(sleep({ hoursSlept: 6, stressLevel: 0 }), 0, []);
  ...
  it('R5: frontera de estrés — exactamente 3 suma 30, exactamente 6 suma 20', () => {
    const at3 = calculateRecoveryScore(sleep({ hoursSlept: 0, stressLevel: 3 }), 0, []);
    const at6 = calculateRecoveryScore(sleep({ hoursSlept: 0, stressLevel: 6 }), 0, []);
  ...
  it('R8: frontera de clasificación — 80/79, 60/59 y 40/39 caen en el estado correcto', () => {
    expect(calculateRecoveryScore(sleep({ hoursSlept: 8, stressLevel: 3 }), 0, workouts(4)).status).toBe('Excelente'); // 40+30+20+10=100 ≥80
    expect(calculateRecoveryScore(sleep({ hoursSlept: 7, stressLevel: 4 }), 0, workouts(1)).status).toBe('Bueno'); // 30+20+20+8=78, entre 60 y 79
    expect(calculateRecoveryScore(sleep({ hoursSlept: 5, stressLevel: 8 }), 1, [])).toMatchObject({ status: 'Riesgo' }); // 10+10+5+0=25 <40
  });
});
